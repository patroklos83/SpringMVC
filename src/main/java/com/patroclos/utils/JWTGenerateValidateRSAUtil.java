...
@Component
public class JWTGenerateValidateRSAUtil {

	public Jws<Claims> verifyParseJwt(String jwtString) throws InvalidKeySpecException, NoSuchAlgorithmException {
		PublicKey publicKey = getPublicKey();
		Jws<Claims> parseClaimsJws = Jwts.parser().setSigningKey(publicKey).parseClaimsJws(jwtString);
		/*
		Validating a token
	...
	public String createJwtSigned(String paylod) throws InvalidKeySpecException, NoSuchAlgorithmException {
		return createJwtSigned(paylod,null);
	}

	public String createJwtSigned(String paylod,Map<String,String> customClaims) throws InvalidKeySpecException, NoSuchAlgorithmException {
		PrivateKey privateKey = getPrivateKey();
		long expirationInMillis = 120 * 1000;
		...
		if (customClaims !=null && customClaims.size() > 0) {
			for(Map.Entry<String, String> customClaim: customClaims.entrySet()) {
				builder.claim(customClaim.getKey(),customClaim.getValue());
			}
		}	
		
		builder.compressWith(CompressionCodecs.DEFLATE); //Warning, other libraries might not be able to parse jwt if compressed		
