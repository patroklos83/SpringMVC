...
public class SignatureUtil {
	...
	public SignatureUtil() throws Exception{
	...
	public byte[] sign(String message) throws Exception {
		...
		byte[] strDigitalSignature = Base64.getEncoder().encode(digitalSignature);
		return strDigitalSignature;
	}
	
	public boolean verify(byte[] signatureHash,String message) throws Exception {
		byte[] strSignature = Base64.getDecoder().decode(signatureHash);
		signature.initVerify(getPublicKey());
		signature.update(message.getBytes());
		boolean isCorrect = signature.verify(strSignature);
		return isCorrect;
	}
